    ...
    async def render(
        ...
        *,
        ...
        instructions = [
            ...
            "Cortex — это контекст памяти, а не новый источник канона; persona.md неизменяем.",
            "TESTIMONY, RUMOR и INFERENCE всегда помечай словами «по словам», «я слышал» или «мне показалось».",
            "Не называй источник при attribution_mode=ANONYMOUS или PRIVATE.",
            "Несколько слухов не становятся подтверждением; при конфликте версий обозначь, что есть разные сообщения.",
            ...
            "Не перечисляй источники, статьи, ссылки, названия базы знаний и не добавляй служебный раздел «Источники». Факты произноси как собственные знания Странника.",
