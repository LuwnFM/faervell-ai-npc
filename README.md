...
Релиз 1.0.0 Discord-бота для RP-проекта «Фаервелл». Это цельная пересборка ядра: локальные решения, Cortex/Mimir-память Странника, раздельные индексы механик и лора, единое существо с профессиональными масками, строгая атрибуция слухов и серверная проверка любых последствий.

Странник — управляемый живой NPC: рабочий приоритет — подтверждённая торговля, точная информация, квесты и услуги; короткий социальный диалог остаётся, но не превращается в пустой монолог. Библиотека `behavior-pack/template-library/` содержит 500 проверенных примеров и 12 operational identity-шаблонов для типовых вопросов вроде «кто ты?».

## Release 1.0.0: Cortex/Mimir rebuild

Память не является бесконтрольным «чёрным ящиком». Каждый факт проходит локальный writer и получает источник, scope, confidence и lifecycle. Claims и evidence хранятся отдельно; слухи, свидетельства игрока и подтверждённый канон не смешиваются. Cortex собирается адаптивно под маршрут `CHAT`, `LORE`, `MECHANICS` или `PLANNER` и включает только разрешённые слои: identity core, personal digest, relationships, open threads, testimony и shared-world impressions.

В production включены PostgreSQL 16, pgvector, FTS и Redis scene-lock. Автоматические LLM-вызовы для обслуживания памяти и «фоновой жизни» отключены. Graph memory подготовлен как следующий этап, но по умолчанию выключен. Это сохраняет предсказуемость на VPS с 4 CPU / 8 GB RAM / 60 GB NVMe.

В комплект входят 512 шаблонов, read-only индекс экономики на 144 768 позиций, Fandom knowledge import, синхронизация персонажей из Discord-канала `707461395209256982` и startup-lock Странника в канале `1488544832950374481`.
...
```bash
git clone https://github.com/LuwnFM/faervell-ai-npc.git
cd faervell-ai-npc
cp .env.example .env
nano .env
...
faervell_npc/
  ...
  services/
    ...
    memory/            Cortex/Mimir memory v2: writer, recall, claims, evidence, testimony
...
## Лицензия и запрет использования

Репозиторий может быть публично просмотрен, но это **не означает выдачу лицензии** на код или материалы. Проект распространяется по `FAERVELL AI-NPC PROPRIETARY LICENSE` (`LICENSE`): без предварительного письменного разрешения правообладателя запрещены копирование, форки, переработка, распространение, продажа, публикация производных работ, включение в другой продукт и использование исходников или шаблонов для обучения/дообучения моделей.

Имена, персонажи, лор, карты, экономика, Discord-экспорты, шаблоны и другие материалы Фаервелла дополнительно защищаются правами их владельцев. Компоненты третьих сторон сохраняют свои собственные лицензии; это не даёт разрешения на копирование кода Faervell AI-NPC. Запросы на письменное разрешение направляйте владельцу проекта LuwnFM.
...
## Исторические release notes и совместимость

Релизы v0.7 и v0.8 сохранены в `docs/` как история изменений. Их version-pinned миграторы не запускаются поверх 1.0.0: production использует только идемпотентный `scripts/migrate-v1.0.0.sh`.
...
При включённом startup-lock каждый запуск возвращает Странника в тестовый канал. Команда `/stranger movement_lock enabled:false` снимает ручную блокировку и разрешает weighted-roll перехода; startup-lock при этом остаётся правилом только для следующего перезапуска.
...
## Grounded lore, personality and Discord knowledge
...
- `/stranger behavior_scan action:memory_list` показывает персонажей в памяти; `action:memory_clear character_id:...` очищает только производную память Cortex/Mimir.
