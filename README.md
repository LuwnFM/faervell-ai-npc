...
Релиз 1.0.0 Discord-бота для RP-проекта «Фаервелл». Это цельная пересборка ядра: локальные решения, Cortex-память Странника, раздельные индексы механик и лора, единое существо с профессиональными масками, строгая атрибуция слухов и серверная проверка любых последствий.

Странник — управляемый живой NPC: рабочий приоритет — подтверждённая торговля, точная информация, квесты и услуги; короткий социальный диалог остаётся, но не превращается в пустой монолог. Библиотека `behavior-pack/template-library/` добавляет 500 проверенных примеров и 32 квестовых архетипа.
...
- Cortex/Mimir memory v2: claims, evidence, testimonies, open threads, lifecycle archive и адаптивный контекст без фиксированного лимита символов.
- Точный read-only индекс экономики: 144 768 позиций из 24 государственных таблиц; неизвестная цена возвращается явно, без догадок.
- Периодическая синхронизация анкет персонажей из канала `707461395209256982` раз в 48 часов с деактивацией пропавших анкет без удаления.
...
## v0.8.0: grounded lore, personality and Discord knowledge

- Полная личность Странника загружается из `behavior-pack/persona.md`; исходник хранится в `docs/stranger-persona-source.md`.
- Лоровый ответ сообщает найденную подтверждённую часть, а неизвестные части регистрирует отдельно без выдумывания канона.
- Профессиональная маска влияет на подачу и инициативные предложения, но не запрещает торговлю или типы квестов.
- `/stranger locations_sync` синхронизирует каналы, форумные посты, ветки, описания локаций, длинные описательные сообщения, ссылки Fandom и доверенные новости мира.
- Новости форума `1320514974396973186` принимаются только из структурированных публикаций авторов `855605848105287711` и `331217779019481089`; обсуждение и оффтоп не индексируются.
- `/stranger behavior_scan action:memory_list` показывает персонажей в памяти; `action:memory_clear character_id:...` очищает только производную память.
- `/stranger behavior_scan action:gaps_list` показывает нормализованные пробелы; `action:gaps_clear` очищает их без удаления канона и сырого архива.
- Публичный RP блокирует случайные китайские/корейские вставки, JSON и неподтверждённые лоровые сущности.
